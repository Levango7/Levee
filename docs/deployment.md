...
- **已具备**：共享 PostgreSQL 存储带来的数据一致性；存储层咨询锁（advisory lock）协调。
- **尚未实现**：节点间自动故障转移、跨节点任务调度、成员发现/心跳的持久化。节点角色（`--node-role master|worker`）当前主要用于标识。

因此多节点部署应视为“共享存储 + 多接入点”，可用性提升依赖外部负载均衡与健康检查（`/healthz`）摘除故障节点，而非内置自动切换。启动时的告警日志会重申这一点，请在容量与 SLO 评估中纳入。
