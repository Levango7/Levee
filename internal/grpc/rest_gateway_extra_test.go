...
func TestExtraServicesProtoJSONContract(t *testing.T) {
	...
	// Returns the status code and decoded body. The response body is read
	// and closed here; callers never see the raw *http.Response, so there is
	// no leak surface for bodyclose to chase through call sites.
	post := func(service, method, body string) (int, map[string]any) {
		t.Helper()
		resp, err := http.Post(
	...
	t.Run("ReceiveAlert omits zero-value fields", func(t *testing.T) {
		code, m := post("AlertService", "ReceiveAlert", `{"source":"prom","title":"t"}`)
	...
	t.Run("GetAlertStatus emits int64 as string", func(t *testing.T) {
		code, _ := post("AlertService", "ReceiveAlert", `{"id":"contract-1","source":"prom","title":"t"}`)
		require.Equal(t, http.StatusOK, code)
		code, m := post("AlertService", "GetAlertStatus", `{"id":"contract-1"}`)
		require.Equal(t, http.StatusOK, code)
	...
	t.Run("SendMessage accepts snake_case and camelCase", func(t *testing.T) {
		code, m := post("ConversationService", "SendMessage", `{"user_id":"u1","text":"/help"}`)
		require.Equal(t, http.StatusOK, code)
		assert.NotEmpty(t, m["text"])
		code, m = post("ConversationService", "SendMessage", `{"userId":"u1","text":"/help"}`)
		require.Equal(t, http.StatusOK, code)
		assert.NotEmpty(t, m["text"])
	})
	...
	t.Run("Diagnose response follows protojson", func(t *testing.T) {
		code, m := post("DiagnosisService", "Diagnose", `{"target":"h1"}`)
		require.Equal(t, http.StatusOK, code)
		assert.NotEmpty(t, m["id"])
