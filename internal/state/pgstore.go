...
func (s *PGStore) UpdateRun(ctx context.Context, run *Run) error {
	...
	res, err := s.db.ExecContext(ctx, `UPDATE runs SET
		workflow_name=$1, template_name=$2, params=$3, plan_hash=$4, plan_json=$5, status=$6,
		approval_status=$7, approval_level=$8, updated_at=$9, creator=$10, incident_id=$11
		WHERE id=$12`,
...
func (s *PGStore) UpdateRunStatusIf(ctx context.Context, id string, from string, to string, updatedAt time.Time) (bool, error) {
...
// takeover rationale.
func (s *PGStore) MarkNonTerminalSteps(ctx context.Context, runID string, marker string) (int64, error) {
	res, err := s.db.ExecContext(ctx,
		`UPDATE steps SET status=$1 WHERE run_id=$2 AND status IN ('running','pending')`,
		marker, runID,
	)
	if err != nil {
